...
public class ViewDistancePacketInterceptor {
  ...
  private final java.util.concurrent.ConcurrentHashMap<String, Long> lastLogMs =
  ...
  public ViewDistancePacketInterceptor(
      Provider<FakeChunkService> fakeChunkServiceProvider, Container<Config> configContainer) {
    ...
    listener =
        new PacketListenerAbstract(PacketListenerPriority.NORMAL) {
          ...
          public void onPacketSend(PacketSendEvent event) {
            ...
            try {
              advertisedDistance =
                  fakeChunkServiceProvider.get().getAdvertisedDistance(player.getUniqueId());
            ...
            if (event.getPacketType() == PacketType.Play.Server.UNLOAD_CHUNK) {
              ...
              if (distanceSq <= radius * radius) {
                ...
                debug(player, "unload_cancel", cancelDebug);
              } else {
                String allowDebug =
                    "[EH] allow UNLOAD_CHUNK "
                        + chunkX
                        + ","
                        + chunkZ
                        + " distSq="
                        ...
                        + " target="
                        + effectiveTargetDistance;
                debug(player, "unload_allow", allowDebug);
              }
              return;
            }

            if (event.getPacketType() == PacketType.Play.Server.UPDATE_VIEW_DISTANCE) {
              WrapperPlayServerUpdateViewDistance wrapper =
                  new WrapperPlayServerUpdateViewDistance(event);
              ...
              if (serverRadius < effectiveTargetDistance) {
                ...
                debug(
                    player,
                    "vd_override",
                    "[EH] override viewDistance server="
                        + serverRadius
                        + " target="
                        + effectiveTargetDistance);
                runForPlayer(
                    ...
                    () -> {
                      ...
                      try {
                        ...
                        sp.connection.send(
                            new ClientboundSetChunkCacheRadiusPacket(effectiveTargetDistance));
                      } catch (Throwable t) {
